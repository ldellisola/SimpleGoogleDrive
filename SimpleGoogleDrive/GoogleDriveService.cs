...
public class GoogleDriveService : IDisposable
{
    ...
    private readonly string _fields = "id, name, parents, mimeType, properties, size, trashed,shortcutDetails";
    ...
    public GoogleDriveService(DriveAuthSettings settings, bool usePersistentStorage = false,
        string? persistentStoragePath = default)
    {
    ...
    public async Task<DriveResource?> GetResource(string? id, CancellationToken token = default)
    {
        ...
        var resource = await ResiliencePolicies
            ...
            .ExecuteAsync(async (c) =>
                {
                    ...
                    request.Fields = _fields;
                    return await request.ExecuteAsync(c);
                },
                token
            );
    ...
    private DriveResource? MapToDriveResource(File? resource)
    {
        var driveResource = resource switch
        {
            ...
            {
                Id = resource.MimeType.MimeType() is DriveResource.MimeType.GoogleDriveShortcut
                    ? resource.ShortcutDetails.TargetId
                    : resource.Id,
                Name = resource.Name,
                Parent = resource.Parents?.FirstOrDefault(t => t is not null && _storage.GetPath(t) is not null,
                    resource.Parents?.FirstOrDefault()),
                Type = resource.MimeType.MimeType() is DriveResource.MimeType.GoogleDriveShortcut
                    ? resource.ShortcutDetails.TargetMimeType.MimeType()
                    : resource.MimeType(),
    ...
    public async IAsyncEnumerable<DriveResource> QueryResources(QueryBuilder queryBuilder,
        [EnumeratorCancellation] CancellationToken token = default)
    {
        ...
        do
        {
            FileList result = await ResiliencePolicies.GetExponentialRetryPolicy()
                .ExecuteAsync(async c =>
                {
                    Calls++;
            ...
            foreach (var resource in result.Files)
    ...
    public async Task<DriveResource?> FindFolder(string pathToFolder, QueryBuilder? parameters = default,
    ...
    public Task<DriveResource?> FindFile(string? pathToFile, QueryBuilder? parameters = default,
        CancellationToken token = default)
    {
    ...
    public async Task<DriveResource?> FindResource(string? pathToResource, QueryBuilder? parameters = default,
        CancellationToken token = default)
    {
    ...
    /// <param name="token">Cancellation token</param>
    /// <returns></returns>
    public async Task DeleteResource(string pathToResource, QueryBuilder? parameters = default,
        CancellationToken token = default)
    {
    ...
    public async Task<DriveResource?> CreateFile(FileStream data, string mimeType, string destination,
        Dictionary<string, string>? properties = default,
        Action<long, long>? onProgress = default, Action<Exception>? onFailure = default,
        CancellationToken token = default)
    {
        ArgumentNullException.ThrowIfNull(_service);
        ...
        if (destinationPath is not null)
            parentId = (
                await FindFolder(destinationPath, token: token) ??
                await CreateFolder(destinationPath, token)
            )?.Id;
    ...
    public async Task<DriveResource?> CreateFile(FileInfo file, string destination,
        Dictionary<string, string>? properties = default,
        Action<long, long>? onProgress = default, Action<Exception>? onFailure = default,
        CancellationToken token = default)
    {
        ...
        {
            var mimeType = file.MimeType().GetString();
    ...
    public async Task<bool> DownloadResource(string pathToResource, MemoryStream stream,
    ...
    /// <param name="token"></param>
    /// <returns>true if the pathToResource is valid</returns>
    public async Task<bool> DownloadResource(string pathToResource, string destination,
        Action<long, long?>? onProgress = default,
        Action<Exception>? onFailure = default, CancellationToken token = default)
    {
        var resource = await FindResource(pathToResource, token: token);
    ...
    /// <param name="token"></param>
    /// <returns></returns>
    public async Task DownloadResource(DriveResource resource, string destination,
        Action<long, long?>? onProgress = default,
        Action<Exception>? onFailure = default, CancellationToken token = default)
    {
        var file = new FileInfo(destination);
    ...
    /// <param name="token"></param>
    /// <returns></returns>
    public async Task<DriveResource?> CopyResource(DriveResource resource, string destination,
        CancellationToken token = default)
    {
    ...
    /// <param name="token"></param>
    /// <returns></returns>
    public async Task<DriveResource?> CopyResource(DriveResource resource, DriveResource? destination,
        CancellationToken token = default)
    {
        ArgumentNullException.ThrowIfNull(_service);
    ...
    /// <param name="token"></param>
    /// <returns></returns>
    public async Task<DriveResource?> UpdateResource(DriveResource? resource, Stream content,
        DriveResource.MimeType contentType,
        Action<long, long>? onProgress = default, Action<Exception>? onFailure = default,
        CancellationToken token = default)
    {
        ArgumentNullException.ThrowIfNull(_service);
